...
from code_review_mas.common.utils import get_mcp_server_config
from code_review_mas.mcp import orchestrator_client
...
class WorkflowNode:
    ...
    async def get_planner_resource(self) -> AgentCard | None:
        ...
        config = get_mcp_server_config(server_name="orchestrator")
        async with orchestrator_client.init_session(
            config.host, config.port, config.transport
        ) as session:
            response = await orchestrator_client.find_resource(
                session, 'resource://agent_cards/serena_planning_agent'
            )
    ...
    async def find_agent_for_task(self) -> AgentCard | None:
        ...
        async with orchestrator_client.init_session(
            ...
            result = await orchestrator_client.find_agent(session, self.task)
